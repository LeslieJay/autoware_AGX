...
/**
 * @brief Calculate the minimum distance required to stop given initial velocity and acceleration constraints.
 * @param initial_vel Initial velocity.
 * @param max_acc Maximum acceleration (deceleration).
 * @param min_acc Minimum acceleration (deceleration).
 * @return Minimum stopping distance.
 */
...
/**
 * @brief Calculate the estimation time for a predicted object based on its deceleration.
 * @param predicted_object The predicted object.
 * @param obstacle_filtering_param Parameters for obstacle filtering.
...
/**
 * @brief Convert a geometry_msgs::msg::Point to autoware_utils_geometry::Point2d.
...
/**
 * @brief Resample trajectory points with a given interval.
 * @param traj_points Input trajectory points.
 * @param interval Resampling interval.
 * @return Resampled trajectory points.
 */
...
/**
 * @brief Resample the highest confidence predicted paths.
 * @param predicted_paths Input predicted paths.
 * @param time_interval Time interval for resampling.
 * @param time_horizon Time horizon.
 * @param num_paths Number of paths to select.
 * @return Resampled predicted paths.
 */
...
/**
 * @brief Calculate the x offset to the bumper based on driving direction.
 * @param is_driving_forward Whether the vehicle is driving forward.
 * @param vehicle_info Vehicle information.
 * @return X offset to bumper.
 */
...
/**
 * @brief Calculate the time to reach the collision point.
 * @param odometry Current odometry.
 * @param collision_point Collision point.
 * @param traj_points Trajectory points.
 * @param x_offset_to_bumper X offset to bumper.
 * @param margin_distance Margin distance.
 * @param min_velocity_to_reach_collision_point Minimum velocity.
 * @return Time to reach collision point.
 */
...
/**
 * @brief Calculate braking distance along the trajectory based on obstacle classification.
 * @param label Obstacle classification label.
 * @param lon_vel Longitudinal velocity.
...
/**
 * @brief Create polygon parameters for trajectory collision check.
 * @param trim_trajectory_param Trajectory trimming parameters.
 * @param ego_braking_distance Ego braking distance.
 * @param lateral_margin_param Lateral margin parameters.
 * @param object_velocity Object velocity.
...
/**
 * @brief Initialize the ObstacleStopModule.
 * @param node ROS node.
...
/**
 * @brief Update parameters from ROS parameter events.
...
/**
 * @brief Plan velocity considering obstacles.
 * @param raw_trajectory_points Raw trajectory points.
 * @param smoothed_trajectory_points Smoothed trajectory points (unused).
 * @param planner_data Planner data.
 * @return Velocity planning result.
 */
...
/**
 * @brief Calculate ego's forwarding braking distance.
 * @param traj_points Trajectory points.
 * @param odometry Current odometry.
 * @return Ego braking distance.
 */
...
/**
 * @brief Get the nearest collision point with point cloud.
 * @param traj_points Trajectory points.
 * @param traj_polygons Trajectory polygons.
 * @param point_cloud Point cloud data.
 * @param x_offset_to_bumper X offset to bumper.
 * @param vehicle_info Vehicle information.
 * @return Nearest collision point with distance.
 */
...
/**
 * @brief Filter stop obstacles from predicted objects.
 * @param odometry Current odometry.
 * @param ego_nearest_dist_threshold Ego nearest distance threshold.
 * @param ego_nearest_yaw_threshold Ego nearest yaw threshold.
 * @param predicted_objects_stamp Timestamp of predicted objects.
 * @param traj_points Trajectory points.
 * @param decimated_traj_points Decimated trajectory points.
 * @param objects List of objects.
 * @param vehicle_info Vehicle information.
 * @param x_offset_to_bumper X offset to bumper.
 * @param trajectory_polygon_collision_check Collision check parameters.
 * @return List of stop obstacles.
 */
...
/**
 * @brief Update or insert pointcloud stop candidates.
 * @param nearest_collision_point Nearest collision point.
 * @param traj_points Trajectory points.
 * @param latest_point_cloud_time Latest point cloud time.
 */
...
/**
 * @brief Filter stop obstacles from point cloud.
 * @param odometry Current odometry.
 * @param traj_points Trajectory points.
 * @param decimated_traj_points Decimated trajectory points.
 * @param point_cloud Point cloud data.
...
/**
 * @brief Pick a stop obstacle from a predicted object.
 * @param odometry Current odometry.
 * @param traj_points Trajectory points.
 * @param decimated_traj_points Decimated trajectory points.
 * @param object Predicted object.
 * @param predicted_objects_stamp Timestamp.
 * @param dist_from_obj_poly_to_traj_poly Distance from object to trajectory.
 * @param vehicle_info Vehicle information.
 * @param x_offset_to_bumper X offset to bumper.
 * @param trajectory_polygon_collision_check Collision check parameters.
 * @return Optional stop obstacle.
 */
...
/**
 * @brief Check if obstacle velocity requires fixed stop.
 * @param object Predicted object.
 * @param traj_points Trajectory points.
 * @return True if fixed stop is required.
 */
...
/**
 * @brief Check if the obstacle is a crossing transient obstacle.
 * @param odometry Current odometry.
 * @param traj_points Trajectory points.
 * @param decimated_traj_points Decimated trajectory points.
 * @param object Predicted object.
 * @param x_offset_to_bumper X offset to bumper.
 * @param decimated_traj_polys_with_lat_margin Trajectory polygons.
 * @param collision_point Collision point.
 * @return True if it's a crossing transient obstacle.
 */
...
/**
 * @brief Plan the stop point based on obstacles.
 * @param planner_data Planner data.
 * @param traj_points Trajectory points.
 * @param stop_obstacles List of stop obstacles.
 * @param x_offset_to_bumper X offset to bumper.
  ...
  const std::vector<StopObstacle> & stop_obstacles, const double x_offset_to_bumper)
{
  ...
  // 初始化变量：确定的停止障碍物、零速度距离、期望停止边距
  std::optional<StopObstacle> determined_stop_obstacle{};// 确实会发生碰撞的障碍物
  ...
  for (const auto & stop_obstacle : closest_stop_obstacles) {
    ...
    if (determined_stop_obstacle) {
      ...
      if (
        ...
        continue;  // 已有同类型的障碍物，且距离ego较远，跳过；原因：优先考虑最近的同类型障碍物
      }
    }
    // 更新确定的停止障碍物和相关距离
...
/**
 * @brief Calculate the desired stop margin.
 * @param planner_data Planner data.
 * @param traj_points Trajectory points.
 * @param stop_obstacle Stop obstacle.
 * @param x_offset_to_bumper X offset to bumper.
 * @param ego_segment_idx Ego segment index.
 * @param dist_to_collide_on_ref_traj Distance to collide.
 * @return Desired stop margin.
 */
...
/**
 * @brief Calculate the candidate zero velocity distance for obstacle stop planning
 * 
 * This function computes the distance at which the vehicle should come to a complete stop
 * when encountering an obstacle. It takes into account the collision distance, desired stop
 * margin, and optionally suppresses sudden stops by ensuring the deceleration is acceptable.
 *
 * @param planner_data Shared pointer to const PlannerData containing current vehicle state,
 *                     odometry, and pose information
 * @param traj_points Reference to vector of TrajectoryPoint defining the reference trajectory
 * @param stop_obstacle The obstacle object that requires stopping, containing classification
 *                      and other obstacle-specific information
 * @param dist_to_collide_on_ref_traj Distance from current position to collision point on
 *                                     the reference trajectory
 * @param desired_stop_margin Safety margin to maintain before the collision point
 *
 * @return std::optional<double> The calculated zero velocity distance in meters.
 *         Returns std::nullopt if the stop cannot be executed (e.g., when abandon_to_stop
 *         is enabled and the object is classified as sudden). Returns a valid distance
 *         otherwise, which may be adjusted upward if sudden stop suppression is enabled
 *         and requires a longer stopping distance.
 *
 * @note When suppress_sudden_stop_ is enabled, the function ensures that deceleration
 *       does not exceed acceptable limits based on obstacle classification. If the
 *       acceptable deceleration cannot achieve the target stop position, the stop
 *       distance is extended accordingly.
 */
  ...
  const double dist_to_collide_on_ref_traj, const double desired_stop_margin)
{
  ...
  if (suppress_sudden_stop_) {
    ...
    const auto acceptable_stop_acc = [&]() -> std::optional<double> {
      ...
      // 计算判断突然性的距离：取最小值以确保安全
      // 使用加速度阈值计算的最小停止距离和距离阈值中的较小者
...
/**
 * @brief Hold previous stop if necessary.
 * @param planner_data Planner data.
 * @param traj_points Trajectory points.
 * @param determined_zero_vel_dist Determined zero velocity distance.
 */
...
/**
 * @brief Calculate the stop point.
 * @param planner_data Planner data.
 * @param traj_points Trajectory points.
 * @param x_offset_to_bumper X offset to bumper.
 * @param determined_stop_obstacle Determined stop obstacle.
 * @param determined_zero_vel_dist Determined zero velocity distance.
 * @return Optional stop point.
 */
...
/**
 * @brief Set stop planning debug information.
 * @param determined_stop_obstacle Determined stop obstacle.
 * @param determined_desired_stop_margin Determined desired stop margin.
 */
...
/**
 * @brief Get trajectory polygon for collision detection.
 * @param decimated_traj_points Decimated trajectory points.
 * @param vehicle_info Vehicle information.
 * @param current_ego_pose Current ego pose.
 * @param polygon_param Polygon parameters.
 * @param enable_to_consider_current_pose Enable considering current pose.
 * @param time_to_convergence Time to convergence.
 * @param decimate_trajectory_step_length Step length.
...
/**
 * @brief Check consistency of stop obstacles.
...
/**
 * @brief Calculate margin from obstacle on curve.
 * @param planner_data Planner data.
 * @param traj_points Trajectory points.
 * @param stop_obstacle Stop obstacle.
 * @param x_offset_to_bumper X offset to bumper.
 * @param default_stop_margin Default stop margin.
 * @return Margin from obstacle on curve.
 */
...
/**
 * @brief Get the closest stop obstacles.
 * @param stop_obstacles List of stop obstacles.
 * @return Closest stop obstacles.
 */
...
/**
 * @brief Get decimated trajectory polygons.
 * @param traj_points Trajectory points.
 ...
 * @param ego_nearest_yaw_threshold Ego nearest yaw threshold.
 * @param trajectory_polygon_collision_check Collision check parameters.
 * @return Decimated trajectory polygons.
 */
...
/**
 * @brief Check for outside cut-in obstacle.
 * @param object Predicted object.
 * @param traj_points Trajectory points.
 * @param decimated_traj_points Decimated trajectory points.
 * @param decimated_traj_polys_with_lat_margin Trajectory polygons.
 * @param x_offset_to_bumper X offset to bumper.
 * @param estimation_time Estimation time.
 * @param predicted_objects_stamp Timestamp.
 * @return Optional collision point.
 */
