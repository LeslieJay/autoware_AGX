// Copyright 2025 BYD
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#ifndef TYPES_HPP_
#define TYPES_HPP_

#include <autoware_utils/geometry/boost_geometry.hpp>
#include <rclcpp/time.hpp>

#include <autoware_planning_msgs/msg/trajectory_point.hpp>
#include <geometry_msgs/msg/pose.hpp>

#include <boost/geometry/index/rtree.hpp>

#include <optional>
#include <string>
#include <vector>

namespace autoware::motion_velocity_planner::trailer_pedestrian_stop
{
using TrajectoryPoints = std::vector<autoware_planning_msgs::msg::TrajectoryPoint>;
/** R-tree value: (bounding box, index into trajectory_footprints) */
using BoxIndexPair = std::pair<autoware_utils::Box2d, size_t>;
using Rtree = boost::geometry::index::rtree<BoxIndexPair, boost::geometry::index::rstar<16, 4>>;

struct TrailerParam
{
  /** Fallback when use_dynamic_count is false or no message received yet */
  size_t count{5};
  /** When true, trailer.count is taken from dynamic_count_topic (UInt8) */
  bool use_dynamic_count{false};
  std::string dynamic_count_topic;
  double length{2.0};
  double width{1.2};
  double hitch_gap{0.3};
  double side_margin{0.8};
  double rear_margin{0.5};
};

struct PedestrianParam
{
  bool enable_pedestrian{true};
  bool enable_bicycle{false};
  bool enable_unknown{false};
  double time_horizon{4.0};
  double radius_margin{0.6};
  double min_predicted_path_confidence{0.1};
  double min_approach_speed{0.1};
  bool use_predicted_path{true};
  bool require_lateral_approach{false};
  double max_lateral_approach_distance{6.0};
};

struct StopParam
{
  double stop_distance_buffer{1.0};
  double add_duration_buffer{0.2};
  double remove_duration_buffer{1.0};
  double hysteresis{1.0};
  bool ignore_unavoidable_collisions{false};
};

struct SamplingParam
{
  double trajectory_step_length{0.5};
};

struct PlannerParam
{
  TrailerParam trailer{};
  PedestrianParam pedestrian{};
  StopParam stop{};
  SamplingParam sampling{};
  double ego_longitudinal_offset{};
  double ego_lateral_offset{};
  double ego_rear_longitudinal_offset{};
};

struct EgoData
{
  TrajectoryPoints trajectory{};
  size_t first_trajectory_idx{};
  double longitudinal_offset_to_first_trajectory_idx{};
  geometry_msgs::msg::Pose pose{};
  autoware_utils::MultiPolygon2d trajectory_footprints{};
  /** trajectory index for each entry in trajectory_footprints */
  std::vector<size_t> trajectory_footprint_traj_indices{};
  Rtree rtree{};
  std::optional<geometry_msgs::msg::Pose> earliest_stop_pose{};
};

struct DebugData
{
  autoware_utils::MultiPolygon2d pedestrian_footprints{};
  size_t prev_pedestrian_footprints_nb{};
  autoware_utils::MultiPolygon2d ego_footprints{};
  size_t prev_ego_footprints_nb{};
  std::optional<geometry_msgs::msg::Pose> stop_pose{};
  size_t prev_collisions_nb{};
  double z{};
  void reset_data()
  {
    pedestrian_footprints.clear();
    ego_footprints.clear();
    stop_pose.reset();
  }
};

struct Collision
{
  geometry_msgs::msg::Point point{};
  std::string object_uuid{};
};
}  // namespace autoware::motion_velocity_planner::trailer_pedestrian_stop

#endif  // TYPES_HPP_
